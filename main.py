...
class MyApp(server.App):
    ...
    def getData(self, params):
        url = "http://www.star.nesdis.noaa.gov/smcd/emb/vci/gvix/G04/ts_L1/ByProvince/Mean/L1_Mean_UKR.R%s.txt" % params["area"]
        data = pd.read_csv(url,index_col=False, header=1)
        df = pd.DataFrame(data[data["year"] == int(params['year'])][data["week"] >= int(params['week1'])][data['week'] <= int(params['week2'])][["year", "week", params['index']]])
        return df
